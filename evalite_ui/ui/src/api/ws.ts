 ...
 *
 * Auth note: the browser `WebSocket` constructor cannot set arbitrary
 * HTTP headers (unlike `fetch`), so this client cannot literally send
 * `X-API-Key` as a header the way `src/api/client.ts` does for REST
 * calls. Instead it appends the key as a query parameter:
 * `?api_key=<key>`. `evalite/server/routes/ws.py` accepts the key via
 * `websocket.query_params.get("api_key")` (falling back to the
 * `x-api-key` header if present) — this fallback has landed and is
